...
class Transport:
    def __init__(self,root):
        self.root=root
        self.root.title("Transport Page")
        ...
        title_lbl1=Label(self.root,text="Global Gesses India PVT. LTD.",font=("Georgia Pro Semibold",28),fg="Maroon")
        title_lbl1.place(x=0,y=50,width=1300,height=45)
 
        b1_2=Button(bg_img,text="Entry to Transport",cursor="hand2",command=self.transport,font=("Times New Roman",13,"bold"),bg="PaleGoldenRod",fg="Black") 
        ...
        search_frame.place(x=450,y=100,width=800,height=580)
    ...
    def fetch_data(self):
        conn = mysql.connector.connect(host="localhost",username="root",password = "Pass@1234",database = 'ongc',auth_plugin = 'mysql_native_password')
        my_cursor = conn.cursor()
        my_cursor.execute("select * from Tank_Details where status= 'In Transist'")
            ...
            for obj in barcode:
                ...
                if result >0:
                    try:
                        ...
                        my_cursor.execute("update tank_details set Date =%s, Time = %s,Status = %s where QR_Code_ID =%s",((dString),(tString),("In Transist"),(data)))                      
