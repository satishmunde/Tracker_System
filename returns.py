...
class Return:
    def __init__(self,root):
        ...
        title_lbl1=Label(self.root,text="Gloabal Gesses India PVT. LTD.",font=("Elephant",28),fg="DarkSlateBlue")
        title_lbl1.place(x=0,y=50,width=1300,height=45)
 
        b1_2=Button(bg_img,text="Entry to Return",cursor="hand2",command=self.scanner,font=("Times New Roman",13,"bold"),bg="Teal",fg="Black") 
    ...
    def fetch_data(self):
        conn = mysql.connector.connect(host="localhost",username="root",password = "Pass@1234",database = 'ongc',auth_plugin = 'mysql_native_password')
        my_cursor = conn.cursor()
        my_cursor.execute("select * from Tank_Details where status= 'Stock'")
            ...
            for obj in barcode:
                ...
                if result >0:
                    try:
                        ...
                        my_cursor.execute("update tank_details set Date =%s, Time = %s,Status = %s where QR_Code_ID =%s and Status =%s",((dString),(tString),("Stock"),(data),("Delevered")))                      
                                                                                                                 
                        my_cursor.execute("delete form orders where Tank_id = %s",((data)))
                                        
                        conn.commit()
                        conn.close()
                        messagebox.showinfo("Sucess","Tank Added Successfully in Stock",parent = self.root)
